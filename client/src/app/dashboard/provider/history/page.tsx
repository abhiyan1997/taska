...
const AppointmentsHistory = () => {
    ...
    useEffect(() => {
        const dataFetch= async()=>{
            const res= await axios.get(`http://localhost:8080/gethistory/${id}`)
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-600">View your completed and past appointments</p>
            </div>
            <div className="flex items-center gap-4">
              <Badge variant="outline" className="px-4 py-2">
                <History className="w-4 h-4 mr-2" />
                {data.length} Completed Appointments
              </Badge>
            </div>
          </div>
        </div>
